...
controls属性是一个布尔属性，如果有这个属性，浏览器就会为视频显示增加内置的播放控件，src属性指明视频的地址，autoplay是告诉浏览器一旦有了足够的数据，就开始播放视频，poster是让视频中的某一帧作为饿哦预览图，preload通常用于细粒度地控制视频如何加载，来实现优化

一个视频文件包含两部分，一个视频部分和一个音频部分，每个部分使用一种特定的编码类型来编码
