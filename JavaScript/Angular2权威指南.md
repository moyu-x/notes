...
依赖注入是这样一个系统：它让程序中的某部分可以访问其他部分，而且我们可以配置它们的访问方式

依赖注入是基于控制反转的设计原则

在传统方式下，如果部件A需要依赖部件B，那就意味着A要在内部创建一个B的实例，也就是A依赖于B。但是在依赖反转的机制下，如果需要在部件A中用到部件B，我们应该期待B被传给A

在使用依赖注入的情况下，若果我们准备单独的测试A，我们可以创建一个mock版本的B，并把它注入到A中

在Angular中，依赖注入包括如下三个部分：
...
`provide`配置方法接收两个键值，第一个`provide`键是我们用作这个可注入对象标识的令牌，第二个键值用来指出注入什么以及如何注入。令牌和被注入物并不一定同名

如果要使用工厂注入，就需要写一个返回任意对象的函数，并且工厂还可以拥有自己的依赖

在开发应用的时候，需要经过三步才能进行依赖注入：
...
每个组件都必须在某些`NgModule`中声明过，任何组件都只能从属于一个`NgModule`

如果任何组件想要使用其他组件，它必须首先通过`NgModule`体系获得访问权

我们可以轻松的制作一个私有组件，只要别把它列进`exports`中就行

指定提供者：只要把可注入对象的提供者添加到`NgModule`的`providers`属性中就算指定完成了

## 第九章

MVC一直是构建数据应用的标准模式：模型包含业务逻辑，视图负责显示数据，控制器将所有一切联系在一起。MVC模式并不能很好地直接转化到客户端的网络应用中

## 第十章

使用可观察对象构造数据称为响应式编程。响应式编程是一种处理异步数据流的编程方法。可观察对象是用来实现响应式编程的主要数据结构。

一些关于流的重要概念：

1. 承诺翻出单个值，而流发出多个值
2. 命令式代码“拉取”数据，而响应式流“推送”数据
3. RxJS是函数式的
4. 流是可以组合的
...
`scan`为输入流中的每个元素运行函数并累加出一个值，它会吧每个中间过程中计算出的结果值发出去
