...
在Render函数中，不再需要Vue的内置指令，当然也没法使用它们。

## 第十章

一个模块就是一个js文件，它拥有独立的作用域，里面定义的变量外部是无法获取的。

webpack配置中最重要也是必选的两项是入口和出口。入口的作用时告诉webpack从哪里寻找依赖，并且编译，出口怎生用来配置编译后的文件存储位置和文件名

在webpack的世界里，每个文件都是一个模块。对于不同的模块，需要不同的加载器（Loaders）来处理

在module对象的rules属性中可以指定一系列的loaders，每一个loader都必须包含test和use两个选项

webpack的四个核心概念：入口，出口，加载器和插件

`.vue`的组件是没有名称的，在父组件使用时可以对它自定义，写好了组件，就可以在入口main.js中使用它了。

箭头函数体内的this对象就是定义时所在的对象，而不是使用时所在的对象

## 第十一章

前端路由，即由前端来维护一个路由规则。实现有两种，一种是利用url的hash，就是常说的锚点(#)，Javascript通过`hashChange`事件监听url的改变，另一种就是HTML5的Historry模式，它市url看起来像普通网站那样，但页面并没有跳转，这种模式需要服务端支持，服务端在接收到所有的请求后，都指向同一个html文件。

每个页面对应一个组件，也就是对应一个`.vue`文件

vue-router有两种跳转页面的方法，第一种是使用内置的`<router-link>`组件，它会被渲染为一个`<a>`标签，当跳转页面需要做JavaScript里进行的时候，这时候可以使用第二种跳转方法，使用router实例的方法。

修改标题最好的思路就是，在页面发生路由改变时，统一设置。vue-router提供了导航钩子beforEach和afterEach，它们会在路由即将改变前和改变后触发，所以设置标题可以在beforEach钩子完成

一个组件可以分为数据和视图，数据更新时，视图也会自动更新。在视图中又可以绑定一些事件，它们触发methods里指定的方法，从而可以改变数据、更新视图

Vuex的设计就是用来统一管理组件状态的，它定义饿了一系列规范来使用和操作数据，使组件应用更加高效。

仓库store包含了应用的数据（状态）和操作过程。Vuex里的数据都是响应式的，任何组件使用同一store的数据时，只要store的数据变化，对应的组件也会立即更新

在组件内，来自store的数据只能读取，不能手动修改，改变store中数据的唯一途径就是显示地提交mutations

mutation里不应该异步操作数据，action里面提交的是mutation，并且可以异步操作业务逻辑

涉及到改变数据的，就应该使用 mutations，存在业务逻辑的，就用actions ，至于将业务逻辑放到action里还是Vue组件里完成，就需要根据实际场景选择，当业务足够大的时候，使用modules可以把它们写到不同的文件中，每个module拥有自己的state，getter，mutation，actions，而且可以多层嵌套 
